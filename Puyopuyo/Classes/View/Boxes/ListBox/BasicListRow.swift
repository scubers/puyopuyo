...
public class BasicListRow<Data>: IListRow {
    ...
    private let cellGen: ListViewGenerator<Data>
    ...
    weak public var listSection: IListSection?
    ...
    private func _getCell() -> (ListBoxCell<Data>, UIView?) {
        ...
        var cell = listSection?.listBox?.dequeueReusableCell(withIdentifier: id) as? ListBoxCell<Data>
        if cell == nil {
            cell = ListBoxCell(id: id)
            let box = listSection?.listBox
            let holder = ListContextHolder<Data> { [weak box, weak cell] in
                if let box = box,
                    let cell = cell,
                    let indexpath = box.indexPath(for: cell),
                    let row = box.getRow(at: indexpath) as? BasicListRow<Data> {
    ...
    private func getContext() -> RecycleContext<Data, UITableView>? {
        if let section = listSection {
            return .init(indexPath: indexPath, index: indexPath.row, size: section.getLayoutableContentSize(), data: data, view: section.listBox)
        }
        return nil
    }
