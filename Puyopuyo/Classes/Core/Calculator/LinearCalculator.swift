...
//            print(results.filter { $1 > 1 })
    ...
    private func calculateChildrenCenter(intrinsic: CGSize) {
        ...
        let justifyContent = regulator.justifyContent.hasCrossAligment(for: regDirection) ? regulator.justifyContent : .center
        for index in 0 ..< measures.count {
            ...
            let standardMain: CGFloat = standardLastEnd + calMargin.forward + (calFixedSize.main / 2)
            standardLastEnd = standardMain + (calFixedSize.main / 2) + calMargin.backward

            // 通过标准位置叠加padding和space来计算具体位置
            let calIndex = CGFloat(index)
            let itemSpaceDelta: CGFloat = spaceDelta * calIndex

            var main = standardMain + regCalPadding.forward
            switch format {
            case .leading:
                main += itemSpaceDelta
            case .trailing:
